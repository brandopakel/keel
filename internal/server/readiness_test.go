...
func TestReadinessPreservesTransitionsAndRetriesFailures(t *testing.T) {
	m := &countedMonitor{}
	c := &client{fd: 42}
	operations := []io_multiplexing.Operation{
	...
	want := []io_multiplexing.Event{{Fd: 42, Op: io_multiplexing.OpRead},
		{Fd: 42, Op: io_multiplexing.OpWrite}, {Fd: 42, Op: io_multiplexing.OpNone},
		{Fd: 42, Op: io_multiplexing.OpRead}}
	if !cacheClientInterest {
		want = nil
		for _, op := range operations {
			want = append(want, io_multiplexing.Event{Fd: 42, Op: op})
		}
	}
	require.Equal(t, want, m.events)
	...
	require.Len(t, m.events, len(want)+2, "a failed change must not be cached as successful")
	...
	require.Len(t, m.events, len(want)+3)
}
