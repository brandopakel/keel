...
func sweepStalledClients(now time.Time, mux io_multiplexing.IOMultiplexer, forget func(*client)) int {
	...
	for _, c := range clients {
		...
		switch {
		case c.unanswered():
			// Whether the kernel still had the descriptor decides between two
			// different faults: a registration the loop lost, or a client the
			// loop's own queues forgot while the kernel would have reported it.
			// Asked here, before the close that would erase the answer.
			registered := mux.Forget(c.fd)
			log.Printf("closing client fd=%d: request unanswered for %s (parsed=%d reply=%d partial=%v held=%v deferred=%v queued=%v interest=%d known=%v registered=%v)",
				c.fd, now.Sub(c.lastProgress).Round(time.Second), len(c.cmds), len(c.out), c.buf != nil,
				c.appendHeld, c.appendDeferred, c.readQueued, c.interest, c.interestKnown, registered)
